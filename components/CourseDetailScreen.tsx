...
const CourseDetailScreen = () => {
  ...
  const hasChanges = useCallback(() => {
    ...
    const initChapters =
      typeof initialCourse.chapters === "string"
        ? JSON.parse(initialCourse.chapters)
        : Array.isArray(initialCourse.chapters)
        ? initialCourse.chapters
        : [];

    return (
      course.name !== (initialCourse.name ?? "") ||
      course.price !== (initialCourse.price ?? 0) ||
      course.discount !== (initialCourse.discount ?? 0) ||
      course.category !== (initialCourse.category ?? "") ||
      course.description !== (initialCourse.description ?? "") ||
      (course.project?.description || "") !==
        (initialCourse.project?.description || "") ||
      totalLessons !== (initialCourse.lessoncount ?? 0) ||
      durationStr !== (initialCourse.duration ?? "") ||
      JSON.stringify(chapters) !== JSON.stringify(initChapters)
    );
  ...
  const handleUpdate = async () => {
    ...
    try {
      await dispatch(updateCourseAsync(payload)).unwrap();
      ...
      <View style={styles.form}>
        ...
        <View style={styles.row}>
          ...
          <View style={styles.half}>
            ...
            <TextInput
              style={styles.input}
              value={String(course.discount ?? 0)}
              onChangeText={(t) =>
...
const styles = StyleSheet.create({
  ...
  courseImage: { width: "100%", height: 200, resizeMode: "cover" },
  form: { padding: 16 },
